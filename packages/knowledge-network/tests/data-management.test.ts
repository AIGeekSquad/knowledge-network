...
describe('KnowledgeGraph - Data Management & Lifecycle', () => {
  ...
  describe('updateData() Method', () => {
    it('should update with new nodes', async () => {
    ...
    it('should add new nodes to existing graph', async () => {
      const graph = new KnowledgeGraph(container, basicGraphData);
      await graph.render();
      ...
      await graph.updateData(expandedData);

      const circles = container.querySelectorAll('circle');
      expect(circles).toHaveLength(4);
    }, 10000);

    it('should remove nodes from graph', async () => {
      const graph = new KnowledgeGraph(container, basicGraphData);
      await graph.render();
      ...
      await graph.updateData(reducedData);

      const circles = container.querySelectorAll('circle');
      expect(circles).toHaveLength(2);
    }, 10000);

    it('should handle complete data replacement', async () => {
      const graph = new KnowledgeGraph(container, basicGraphData);
      await graph.render();
    ...
    it('should handle position preservation during updates', async () => {
      const graph = new KnowledgeGraph(container, basicGraphData);
      await graph.render();
      ...
      await expect(graph.updateData(basicGraphData)).resolves.not.toThrow();
    ...
    it('should handle empty data updates', async () => {
      ...
      await expect(graph.updateData(emptyData)).resolves.not.toThrow();
  ...
  describe('getSimulation() Method', () => {
    ...
    it('should return simulation after render', async () => {
      const graph = new KnowledgeGraph(container, basicGraphData);
      await graph.render();
    ...
    it('should return same simulation on multiple calls', async () => {
      const graph = new KnowledgeGraph(container, basicGraphData);
      await graph.render();
